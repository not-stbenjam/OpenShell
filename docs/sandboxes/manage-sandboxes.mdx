...
updated Docker daemon capability. For bare `--gpu` requests, Docker and Podman
select one NVIDIA CDI GPU from the gateway-local inventory. Use `--gpu-device`
with a CDI ID such as `nvidia.com/gpu=0` when you need a specific device.
