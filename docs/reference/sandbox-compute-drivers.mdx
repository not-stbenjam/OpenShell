...
For GPU-backed Docker sandboxes, configure Docker CDI before starting the
gateway so OpenShell can detect the daemon capability. A sandbox with `--gpu`
and no `--gpu-device` gets one default NVIDIA CDI device from Docker daemon
inventory. If Docker reports CDI support but no discovered NVIDIA CDI devices,
bare `--gpu` requests fail; explicit `--gpu-device nvidia.com/gpu=...`
requests still pass through to Docker.
...
For GPU-backed local Linux Podman sandboxes, OpenShell maps local
`/dev/nvidiaN` device nodes to `nvidia.com/gpu=N` inventory entries. A bare
`--gpu` request selects one of those entries. Remote Podman and Podman-machine
CDI device inventory reporting is not currently available through Podman, so
bare `--gpu` cannot infer remote devices until Podman exposes that inventory.
