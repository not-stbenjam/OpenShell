...
| CDI GPU request | Uses explicit `resource_requirements.gpu.device_ids` when set; otherwise selects one or `resource_requirements.gpu.count` NVIDIA CDI devices from Docker's discovered CDI inventory. |
