...
| CDI GPU devices | Explicit `resource_requirements.gpu.device_ids` when set; otherwise one or `resource_requirements.gpu.count` NVIDIA CDI devices selected from local `/dev/nvidiaN` inventory. | Exposes requested GPUs to GPU-enabled sandbox containers. |
