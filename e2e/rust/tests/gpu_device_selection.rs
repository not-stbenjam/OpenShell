...
#[test]
fn default_cdi_gpu_device_id_prefers_indexed_family() {
    assert_eq!(
        default_cdi_gpu_device_id(&[
            "nvidia.com/gpu=UUID-b".to_string(),
            "nvidia.com/gpu=10".to_string(),
            "nvidia.com/gpu=2".to_string(),
            CDI_GPU_DEVICE_ALL.to_string(),
        ]),
        Some("nvidia.com/gpu=2".to_string())
    );
}

#[test]
fn default_cdi_gpu_device_id_uses_uuid_family_when_no_indexed_ids_exist() {
    assert_eq!(
        default_cdi_gpu_device_id(&[
            "nvidia.com/gpu=UUID-b".to_string(),
            "nvidia.com/gpu=UUID-a".to_string(),
        ]),
        Some("nvidia.com/gpu=UUID-a".to_string())
    );
}

#[test]
fn default_cdi_gpu_device_id_uses_all_only_inventory() {
    assert_eq!(
        default_cdi_gpu_device_id(&[CDI_GPU_DEVICE_ALL.to_string()]),
        Some(CDI_GPU_DEVICE_ALL.to_string())
    );
}
